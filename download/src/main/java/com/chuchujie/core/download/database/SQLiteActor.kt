...
open class SQLiteActor(context: Context) : DbActor {
    ...
    private val CURRENT_SIZE = "current_size"
    private val TOTAL_SIZE = "total_size"
    private val STATUS_FLAG = "status_flag"

    private val sqLiteOpenHelper = object : SQLiteOpenHelper(context, DATABASE_NAME, null,
            DATABASE_VERSION) {
    ...
    override fun isExists(mission: RealMission): Boolean {
        ...
        var exist = false
        sqLiteOpenHelper.readableDatabase.use { readableDatabase ->
    ...
    override fun create(mission: RealMission) {
        ...
        sqLiteOpenHelper.writableDatabase.use { writableDatabase ->
    ...
    override fun update(mission: RealMission) {
        ...
        sqLiteOpenHelper.writableDatabase.use { writableDatabase ->
            writableDatabase.update(TABLE_NAME, cv, "$TAG=?",
    ...
    open fun onTransformStatus(status: Status): Int = when (status) {
        is Normal -> 1
        is Suspend -> 2
        is Failed -> 3
        is Succeed -> 4
    ...
    open fun onRestoreStatus(flag: Int, status: Status): Status = when (flag) {
        1 -> Normal(status)
        2 -> Suspend(status)
    ...
    override fun getAllMission(): Maybe<List<Mission>> {
        ...
        }.subscribeOn(newThread()).doOnError { e("get all mission error", it) }
    }
    ...
    private fun transformFlagToInt(rangeFlag: Boolean?): Int = when (rangeFlag) {
        true -> RANGE_FLAG_TRUE
        false -> RANGE_FLAG_FALSE
        else -> -RANGE_FLAG_NULL
    }

    private fun transformFlagToBool(flag: Int): Boolean? = when (flag) {
